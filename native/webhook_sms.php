...
$saveData = $_POST;
	file_put_contents('log/SMS'.date("dmY"),json_encode($saveData).PHP_EOL, FILE_APPEND);
	// file_put_contents('log/'.date("dmY")."_Result",json_encode($result).PHP_EOL, FILE_APPEND);
	file_put_contents('log/SMS'.date("dmY")."_GET",json_encode($_GET).PHP_EOL, FILE_APPEND);
	// echo "mantap ";
