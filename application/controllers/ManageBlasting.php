...
class ManageBlasting     extends CI_Controller
{
    function __construct()
    {
        ...
        //2iNvy9zUUVSwMXSO71SIvdNwjE2c7DrfV6Kn3tCRcOvrkMnvl74kraCUbhZAHZZO
        $this->client               = new GuzzleHttp\Client();
        $this->startRes             = time();
        $this->hotline_status       = "";
        $this->hotline_name         = "";
        $this->hotline_device_id    = "";
        $this->hotline_device_name  = "";
        $this->hotline_domain_api   = "";
        $this->hotline_api          = "";
        $this->hotline_token        = "";
        $this->hotline_phone        = "";
    ...
    public function getHotline(){
        ...
        $customer   = $this->ManageBlasting_model->get_hotline($company_id);
        if($customer){
            // print_r($customer);
            $this->hotline_status       = $customer->wa_status;
            $this->hotline_name         = $customer->name;
            $this->hotline_device_id    = $customer->device_id;
            $this->hotline_device_name  = $customer->device_name;
            $this->hotline_domain_api   = $customer->domain_api;
            $this->hotline_token        = $customer->token;
            $this->hotline_phone        = $customer->phone_number;
        }
    }
    ...
    public function index()
    {
        ...
        $this->template->load('template','ManageBlasting/index', $data);
    } 
    
    public function json() {
        header('Content-Type: application/json');
        echo $this->ManageBlasting_model->json();
    }

    public function get_contact(){
        header('Content-Type: application/json');
        echo $this->ManageBlasting_model->json_getcontact();
    }

    public function read($id) 
    {
        $row = $this->ManageBlasting_model->get_by_id($id);
        if ($row) {
            echo json_encode([
                "code" => "success",
    ...
    public function sendMessage($number, $message, $type = 'random') {
        $this->Loging("sendMessage_response_wablas" , $this->hotline_domain_api);
       
       try {
            $response = $this->client->request( 'POST', 
                                           $this->hotline_domain_api."/send-message", 

                                          [ 
                                            'headers' => [
                                                                'Accept' => 'application/json',
                                                                'Authorization' => $this->hotline_token,
                                                            ],
                                            'form_params' 
                                                => [
                                                'phone'     => $number,
            ...
            $id = $this->ManageBlasting_model->insert_header($dataInsert);
            ...
            foreach ($res['body']['data']['message'] as $key => $value) {
                ...
                $this->ManageBlasting_model->insert($data);
            }
    ...
    public function sendImg($data, $type = 'random') {
       ...
       try {
        $url =  $this->hotline_domain_api."/send-image";
        if ($data['type_file'] == "doc_file") {
            $url =  $this->hotline_domain_api."/send-document";
        }
            $response = $this->client->request( 'POST', 
                                          $url, 
                                          [ 
    ...
    public function sendImgLocal($number,$message,$fileData, $type = 'random') {
       ...
       try {
            $response = $this->client->request( 'POST', 
                                           $this->hotline_domain_api."/send-image-from-local", 
                                          [ 
                                            'headers' => [
                                                                'Accept' => 'application/json',
                                                                'Authorization' => $this->hotline_token,
            ...
            foreach ($body['data']['message'] as $key => $value) {
                ...
                $idImg = $this->ManageBlasting_model->insert($data);
